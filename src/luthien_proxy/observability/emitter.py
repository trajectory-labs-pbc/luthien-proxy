...
def _is_attribute_sequence(value: list[Any]) -> bool:
    """Whether ``value`` is a sequence the SDK's ``_clean_attribute`` accepts.

    The SDK's rule, mirrored here so the filter is neither looser nor stricter than what
    ``add_event`` will take: None elements are allowed and skipped for typing; every other
    element must be an attribute scalar, and all of them must be the same exact type as the
    first non-None one (``type(x) is type(y)``, so ``True`` and ``1`` do not mix). An empty
    list, or one of only Nones, is accepted.
    """
    first_type: type | None = None
    for item in value:
        if item is None:
            continue
        if not isinstance(item, _SPAN_ATTRIBUTE_SCALARS):
            return False
...
def _span_event_attributes(transaction_id: str, safe_data: dict[str, Any]) -> dict[str, Any]:
    ...
    leaves request bodies to the sinks built to hold them: a None value is skipped, scalars
    and the lists the SDK itself accepts (``_is_attribute_sequence``) pass through,
    ...
    for key, value in safe_data.items():
        ...
        if isinstance(value, _SPAN_ATTRIBUTE_SCALARS):
            ...
            continue
        if isinstance(value, list) and _is_attribute_sequence(value):
            attributes[key] = value
    return attributes
